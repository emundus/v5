	...
	*/
	...
	function getInput()
	{
		...
		$menutype = $db->loadResult();
			$query = 'SELECT m.menutype, m.id, m.title, m.parent_id, m.link FROM #__menu m WHERE m.parent_id = 1 AND m.menutype="'.$menutype.'" ORDER BY m.parent_id DESC, m.lft, m.level, m.menutype, m.id ASC';
			$db->setQuery($query);
			...
			foreach($parents as $parent){
				...
				$query = 'SELECT m.menutype, m.id, m.title, m.parent_id, m.link FROM #__menu m WHERE m.parent_id = '.$parent->id.' ORDER BY m.parent_id DESC, m.lft, m.level, m.menutype, m.id ASC';
				$db->setQuery($query);
