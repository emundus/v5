...
if(!empty($this->users)) {
	...
	//echo '<span class="editlinktip hasTip" title="'.JText::_('EXPORT_COMPLETED_TO_XLS').'"><a rel="{handler:\'iframe\',size:{x:window.getWidth()*0.8,y:window.getHeight()*0.8}}" href="'.$this->baseurl.'/index.php?option=com_emundus&view=select_elements&tmpl=component" target="_self" class="modal"><img src="'.$this->baseurl.'/images/emundus/icones/XLSFile_48.png" name="export_complete_to_xls" onclick="document.pressed=this.name" /></a></span>'; 
	...
	if (isset($user->Final_grade)) {
		...
		if ($user->Final_grade!= -1 && $user->Final_grade != '') {
			if ($user->Final_grade< 3)
				$final_grade = '<img src="'.$this->baseurl.'/images/emundus/icones/fileclose.png" alt="'.JText::_($fg).'" title="'.JText::_($fg).'" width="16" height="16" align="absbottom" /> ';
			elseif ($user->Final_grade == 3) 
				$final_grade = '<img src="'.$this->baseurl.'/images/emundus/icones/kalarm_16x16.png" alt="'.JText::_($fg).'" title="'.JText::_($fg).'" width="16" height="16" align="absbottom" /> ';
			elseif ($user->Final_grade == 4) 
				$final_grade = '<img src="'.$this->baseurl.'/images/emundus/icones/icq_ffc.png" alt="'.JText::_($fg).'" title="'.JText::_($fg).'" width="16" height="16" align="absbottom" /> ';
