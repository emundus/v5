...
if(!EmundusHelperAccess::isCoordinator($current_user->id)) {
	...
	$experts_list = $evaluations->getExperts($student_id, $reference_field, $reference_table);
	$experts_email = array();
	foreach ($experts_list as $key => $value) { 
...
<h1><?php echo JText::_( 'INFORM_EXPERTS' ); ?></h1>
	<div id="attachment_list">
	  <form id="adminForm" name="adminForm" onSubmit="return OnSubmitForm();" method="POST" enctype="multipart/form-data" />
	    <?php echo EmundusHelperEmails::createEmailBlock(array('expert')); ?>
	  </form>
	</div>

	<?php 
	$attachments = $evaluations->getEvaluationDocuments($student_id, $campaign_id, 0); 
//var_dump($attachments);
	if ( count($attachments) == 0 ) {
		require(JPATH_LIBRARIES.DS.'emundus'.DS.'pdf.php'); 
		$files = letter_pdf($user->id, 0, $campaign['training'], $campaign_id, 0, "F");
	} else {
		//var_dump($attachments);
...
/////////////////////////////////////////
	...
	function OnSubmitForm() {
		...
		switch(document.pressed) {
			case 'expert': 
				document.adminForm.action ="index.php?option=com_emundus&task=sendmail_expert&sid=<?php echo $student_id ?>&Itemid=<?php echo $itemid ?>";
			break;
