...
<input type="hidden" name="view" value="eval_auto"/>
...
<?php 
	if($tmpl == 'component') {
			echo '<div><h3><img src="'.JURI::Base().'images/emundus/icones/folder_documents.png" alt="'.JText::_('EVALUATION_AUTO').'"/>'.JText::_('EVALUATION_AUTO').' : '.$this->schoolyear.'</h3>';
			$document =& JFactory::getDocument();
			$document->addStyleSheet( JURI::base()."components/com_emundus/style/emundusraw.css" );
	}else{
			echo '<fieldset><legend><img src="'.JURI::Base().'images/emundus/icones/folder_documents.png" alt="'.JText::_('EVALUATION_AUTO').'"/>'.JText::_('EVALUATION_AUTO').' : '.$this->schoolyear.'</legend>';
	}
?>
	...
	<thead>
        ...
        <tr>
            ...
            foreach ($this->users[0] as $key=>$value){ 
				...
				switch($key){
					case 'user_id':?>
                        <th align="center" style="font-size:9px;">
					...
					break;
					case 'name' : 
					 	echo '<th>'.JHTML::_('grid.sort', JText::_($key), $key, $this->lists['order_Dir'], $this->lists['order']).'</th>';
					break; 
					case 'category' : 
					 	echo '<th>'.JHTML::_('grid.sort', JText::_($key), $key, $this->lists['order_Dir'], $this->lists['order']).'</th>';
					break; 
					...
					break;
					default:
						if($key!='final_grade' && $key!='row_id' && $key!='r' && $key!='highest_diploma' && $key!='highest_diploma_univ' && $key!='highest_diploma_city') {
		...
		foreach($this->users as $evalu){
			...
			<tr class="row<?php echo $j++%2; ?>"><?php
				foreach ($evalu as $key=>$value){
					...
					}else if ($key!='final_grade' && $key!='row_id' && $key!='highest_diploma' && $key!='highest_diploma_univ' && $key!='highest_diploma_city'){
						if(($key!='name') && ($key!='nationality') && ($key!='application') && ($key!='overall') && ($key!='r') && ($key!='highest_diploma') && ($key!='highest_diploma_univ') && ($key!='highest_diploma_city')){
							if($value || $value === '0') $class = "class=green";
							else $class = "class=red";		
							$td = '<td '.$class.'>';
						}
						else $td = '<td>';
						echo $td.' '.mb_strtolower($value,'UTF-8');?>	 
						</td><?php 
					} 
				} ?>
                		 <?php if($current_user->profile!="16"){ ?> <td><?php 
		$query = 'SELECT id FROM #__emundus_evaluations WHERE student_id  ='.$evalu['user_id'];
		$db->setQuery( $query ); 
		$row = $db->loadObject();
				...
				?>
						<td>
                            ...
                            $query = 'SELECT ege.id, ege.group_id, ege.user_id  
                                        ...
                                        WHERE ege.applicant_id='.$evalu['user_id'];
                            $db->setQuery( $query );
                            ...
                            foreach($assessors as $ass) {
								...
								if(!empty($ass->group_id) && isset($ass->group_id)) {
                                    ...
                                    $uList .= '</ul>';
                                    $img = '<span class="editlinktip hasTip" title="'.JText::_('DELETE_ASSESSOR').' : '.$this->groups[$ass->group_id]->label.'::'.JText::_('DELETE_ASSESSOR_TXT').'"><a href="index.php?option=com_emundus&controller=eval_auto&task=delassessor&aid='.$evalu['user_id'].'&pid='.$ass->group_id.'&uid='.$ass->user_id.'&limitstart='.$ls.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir.'"><img src="'.JURI::Base().'images/emundus/icones/clear_left_16x16.png" alt="'.JText::_('DEL_ASSESSOR').'" align="absbottom" /></a></span> ';
                                    
                                      if($current_user->profile!="16"){ echo '<span class="editlinktip hasTip" title="'.JText::_('GROUP_MEMBERS').'::'.$uList.'">'.$this->groups[$ass->group_id]->label.'</span> '.$img.'<br />'; }
									  else  echo '<span class="editlinktip hasTip" title="'.JText::_('GROUP_MEMBERS').'::'.$uList.'">'.$this->groups[$ass->group_id]->label.'</span></br>';
                                    unset($uList);
                                } else if(!empty($ass->user_id) && isset($ass->user_id)) {
									//print_r($this->evalUsers);
									$usr =& JUser::getInstance($ass->user_id); 
                                    $img = '<span class="editlinktip hasTip" title="'.JText::_('DELETE_ASSESSOR').' : '.$this->evalUsers[$ass->user_id]->name.'::'.JText::_('DELETE_ASSESSOR_TXT').'">
												<a href="index.php?option=com_emundus&controller=eval_auto&task=delassessor&aid='.$evalu['user_id'].'&pid='.$ass->group_id.'&uid='.$ass->user_id.'&limitstart='.$ls.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir.'"><img src="'.JURI::Base().'images/emundus/icones/clear_left_16x16.png" alt="'.JText::_('DEL_ASSESSOR').'" align="absbottom" />
												</a>
											</span> ';
                                    if($current_user->profile!="16"){ echo $this->evalUsers[$ass->user_id]->name.' '.$img.'<br />';	}
									else  echo $this->evalUsers[$ass->user_id]->name.'</br>';
                                }
                            }
...
function OnSubmitForm() {
	...
	switch(button_name[0]) {
	    case 'affect': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=setAssessor";
		break;
		case 'unaffect': 
			if (confirm("<?php echo JText::_("CONFIRM_UNAFFECT_ASSESSORS"); ?>")) {
        		document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=unsetAssessor";
		 	} else 
		 		return false;
		break;
		case 'export_zip': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=export_zip";
		break;
		case 'export_to_xls': 
			document.adminForm.action ="index.php?option=com_emundus&task=transfert_view&v=<?php echo $v; ?>&Itemid=<?php echo $itemid; ?>";
		break;
		case 'set_status': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=administrative_check&limitstart=<?php echo $ls; ?>";
		break;
		case 'validate': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=validate&uid="+button_name[1]+"&limitstart=<?php echo $ls; ?>";
		break;
		case 'unvalidate': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=unvalidate&uid="+button_name[1]+"&limitstart=<?php echo $ls; ?>";
		break;
		case 'custom_email': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=customEmail";
		break;
		case 'default_email': 
			if (confirm("<?php echo JText::_("CONFIRM_DEFAULT_EMAIL"); ?>")) {
        		document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=defaultEmail";
		 	} else 
		 		return false;
		break;
		case 'view_calc': 
			document.adminForm.action ="index.php?option=com_emundus&view=eval_auto";
		break;
		case 'search_button': 
			document.adminForm.action ="index.php?option=com_emundus&view=eval_auto";
		break;
		case 'clear_button': 
			document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=clear";
		break;
		case 'delete_eval': 
			if(confirm('Are you sure ?'))
				document.adminForm.action ="index.php?option=com_emundus&controller=eval_auto&task=delete_eval&sid="+button_name[1];
			else return false;
		break;
		case 'delete':
		if(confirm('Are you sure ?'))
			document.adminForm.action = "index.php?option=com_emundus&controller=eval_auto&task=delete&sid="+button_name[1];
		break;
...
function delayAct(user_id){
	document.adminForm.action = "index.php?option=com_emundus&view=eval_auto#cb"+user_id;
