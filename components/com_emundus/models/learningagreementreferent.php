...
class EmundusModelLearningAgreementReferent extends JModel
{
	var $_total = null;
	...
	function _buildQuery()
	{
		...
		$pid = JRequest::getVar('pid', null, 'POST', 'none', 0);
		...
		if((isset($uid) && !empty($uid)) || $current_user->profile > 5) 
			...
			$query .= 'WHERE eu.profile IN (7, 8, 10, 11) ';// profile=8 : selected applicant :: profile=7 : Enrolled student
			...
			if(isset($pid) && !empty($pid)) {
				...
				$query.= 'eu.profile='.mysql_real_escape_string($pid);
			}
