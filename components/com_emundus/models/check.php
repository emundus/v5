...
class EmundusModelCheck extends JModel
{
    ...
    function __construct()
    {
        ...
        */
        ...
        $filter_order_Dir =
            $mainframe->getUserStateFromRequest($option . 'filter_order_Dir', 'filter_order_Dir', 'desc', 'word');
        $schoolyears =
            $mainframe->getUserStateFromRequest($option . 'schoolyears', 'schoolyears', $this->getCurrentCampaign());
        $campaigns =
            $mainframe->getUserStateFromRequest($option . 'campaigns', 'campaigns', $this->getCurrentCampaignsID());
        ...
        $elements_values_other =
            $mainframe->getUserStateFromRequest($option . 'elements_values_other', 'elements_values_other');
        $finalgrade =
            $mainframe->getUserStateFromRequest($option . 'finalgrade', 'finalgrade', $filts_details['finalgrade']);
        ...
        $missing_doc =
            $mainframe->getUserStateFromRequest($option . 'missing_doc', 'missing_doc', $filts_details['missing_doc']);
...
//	print_r($this->_applicants);
    ...
    function _buildSelect(&$tables_list, &$tables_list_other, &$tables_list_default)
    {
        ...
        if (empty($schoolyears)) $query .=
            ' AND #__emundus_campaign_candidature.year IN ("' . implode('","', $this->getCurrentCampaign()) . '")';
    ...
    function _buildFilters($tables_list, $tables_list_other, $tables_list_default)
    {
        ...
        elseif (!empty($campaigns))
            ...
            $query .=
                ' AND #__emundus_setup_campaigns.id IN ("' . implode('","', $this->getCurrentCampaignsID()) . '")';
        ...
        if (isset($gid) && !empty($gid)) {
            ...
            $query .= ' (#__emundus_groups_eval.group_id=' . mysql_real_escape_string($gid) .
                ' OR #__emundus_groups_eval.user_id IN (select user_id FROM #__emundus_groups WHERE group_id=' .
        ...
        if (isset($uid) && !empty($uid)) {
            ...
            $query .= ' (#__emundus_groups_eval.user_id=' . mysql_real_escape_string($uid) .
                ' OR #__emundus_groups_eval.group_id IN (select e.group_id FROM #__emundus_groups e WHERE e.user_id=' .
        ...
        if (isset($profile) && !empty($profile)) {
            ...
            $query .=
                ' (#__emundus_setup_profiles.id = ' . $profile . ' OR #__emundus_final_grade.result_for = ' . $profile .
                ' OR #__emundus_users.user_id IN (select user_id from #__emundus_users_profiles where profile_id = ' .
                $profile . '))';
        }
...
// echo str_replace("#_", "jos", $query);
    ...
    function getPagination()
    {
        ...
        if (empty($this->_pagination)) {
            ...
            $this->_pagination =
                new JPagination($this->getTotal(), $this->getState('limitstart'), $this->getState('limit'));
        }
