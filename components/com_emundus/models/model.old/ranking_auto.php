...
class EmundusModelRanking_auto extends JModel
{
	var $_total = null;
	...
	function _buildContentOrderBy(){
		...
		$sort=($filter_order_Dir=='desc')?SORT_DESC:SORT_ASC;
 		$can_be_ordering = array ('user_id', 'user_name', 'nationality','category', 'profile', 'scholarship','application','interview','english','french', 'file2', 'result_for', 'overall','r');
	...
	function _buildQuery(){
	    ...
	    $query='SELECT type, calc from `#__emundus_evaluation_weight` where calc<>"" and alias in (1,2,3,4,5,6) order by alias';
		$db->setQuery($query);
		$eval_type=$db->loadObjectlist();
		//print_r($eval_type);
		//////////////////////////////////////////////
		$query = 'SELECT ed.user,efg.Final_grade,efg.id, epd.nationality, eu.profile, efg.scholarship, efg.result_for, u.email,
					epd.last_name, epd.first_name, epd.gender, substr(epd.birth_date,1,4) as birth_year, epd.street_1, epd.zipcode_1, epd.city_1, epd.country_1, epd.telephone_1, epd.accompanying, epd.nb_children, ea.Institution_1, ea.Country_1 
		...
		if(empty($schoolyears)) $query .= ' AND eu.schoolyear="'.$this->getSchoolyear().'"';// profile=9=applicant
		...
		} else if ($current_user->usertype=='Author') {
		...
		foreach($applicants as $applicant){
			...
			$query = 'SELECT esp.label
				FROM #__emundus_setup_profiles esp
				JOIN #__emundus_final_grade efg ON esp.id = efg.result_for
				WHERE efg.student_id='.$user_id;
			$db->setQuery($query);
			$label=$db->loadResult();
			
			if(empty($view_calc)) {
				
				$eval_list['Family Name'] = $applicant->last_name;
				$eval_list['First name'] = $applicant->first_name;
				$eval_list['Email'] = $applicant->email;
				$eval_list['Nationality']=$applicant->nationality;
				$eval_list['Gender'] = $applicant->gender;
				$eval_list['Profile']=$applicant->profile;
				$eval_list['result_for'] = $applicant->result_for;
				$eval_list['Year of birth'] = $applicant->birth_year;
				$eval_list['Address'] = $applicant->street_1;
				$eval_list['Postal Code'] = $applicant->zipcode_1;
				$eval_list['City'] = $applicant->city_1;
				$eval_list['Country of residence'] = $applicant->country_1;
				$eval_list['Phone'] = $applicant->telephone_1;
				$eval_list['Life partner'] = $applicant->accompanying;
				$eval_list['Children'] = $applicant->nb_children;
				$eval_list['Institution delivering previous degree'] = $applicant->Institution_1;
				$eval_list['Country of University of origin'] = $applicant->Country_1;
				
				$eval_list['category']=$applicant->scholarship;	
			}
			...
			foreach($eval_type as $eval){
				...
				} else $grade = !empty($view_calc)?$str:substr(@eval($str), 0, 6);
	...
	function getTableColumns(){
		$db =& JFactory::getDBO();
		$query = 'SELECT name, type FROM #__emundus_evaluation_weight WHERE parent_id = 0';
		$db->setQuery( $query );
		return $db->loadObjectList('type');
	}
