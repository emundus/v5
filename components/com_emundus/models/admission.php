...
class EmundusModelAdmission extends JModel
{
	...
	function _buildQuery()
	{
		...
		$selected = JRequest::getVar( 'checkselected', array(), 'post', 'array' );
                $schoolYear = JRequest::getVar('schoolyear', $this->getCampaign(), 'POST', 'string', 0);
...
//			}
                        if(!empty($schoolYear))
                            $query .= ' AND eu.schoolyear = "'.$schoolYear.'"';
...
//		echo str_replace('#_', 'jos',$query).'<br /><br />';
        ...
        function getCampaign()
	{
        ...
        /*
         * Retourne l'ensemble des années scolaires des etudiants deja inscrits pour filtrage
         */
	function getAllCampaigns()
	{
		$db =& JFactory::getDBO();
		$query = "SELECT distinct(schoolyear) FROM #__emundus_users where schoolyear > '' order by schoolyear desc;";
		$db->setQuery( $query );
	...
	function getElements()
	{
		...
		$query = 'SELECT element.id, element.name AS element_name, element.label AS element_label, element.plugin AS element_plugin,
				 groupe.label AS group_label, INSTR(groupe.attribs,"repeat_group_button=1") AS group_repeated,
			...
			FROM jos_fabrik_elements element	
				 ...
				 INNER JOIN jos_fabrik_tables AS tab ON tab.form_id = formgroup.form_id
				 INNER JOIN jos_menu AS menu ON tab.id = SUBSTRING(menu.link , LOCATE("tableid=",menu.link)+8 , 2)
				 INNER JOIN jos_emundus_setup_profiles AS profile ON profile.menutype = menu.menutype
			WHERE tab.state = 1 AND profile.id =9 AND tab.created_by_alias = "form" AND element.state=1 AND element.hidden=0 AND element.label!=" " AND element.label!="" 
