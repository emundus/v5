...
class EmundusModelChecklist extends JModel
{
	var $_user = null;
	var $_db = null;
	var $_need = 0;
	...
	function getAttachmentsList()
	{
		$query = 'SELECT attachments.id, COUNT(uploads.attachment_id) AS nb, attachments.nbmax, attachments.value, attachments.lbl, attachments.description, attachments.allowed_types, profiles.mandatory
					FROM #__emundus_setup_attachments AS attachments
						...
						LEFT JOIN #__emundus_uploads AS uploads ON uploads.attachment_id = profiles.attachment_id AND uploads.user_id = '.$this->_user->id.'
		...
		foreach($attachments as &$attachment) {
			if($attachment->nb>0) {
				$this->_db->setQuery('SELECT id, filename, description, can_be_deleted, can_be_viewed FROM jos_emundus_uploads WHERE user_id = '.$this->_user->id.' AND attachment_id = '.$attachment->id);
