 ...
 * @subpackage Components
 */
class EmundusControllerEval_auto extends JController {
	...
	function export_selected_xls(){
		 ...
		 else {$this->setRedirect("index.php?option=com_emundus&view=eval_auto&limitstart=".$page,JText::_("NO_ITEM_SELECTED"),'error');}
	}
   ...
   ////// EXPORT ALL XLS ///////////////////	
	function export_to_xls($reqids=array()) {
		...
		require_once('libraries/emundus/xls_eval_auto.php');
		$profile = JRequest::getVar('profile', null, 'POST', 'none',0);
		export_applicants($reqids);
	...
	function setAssessor($reqids = null) {
		...
		if (count($ids)>1)
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('MESSAGE_APPLICANTS_AFFECTED').count($ids), 'message');
		elseif (count($ids)==1)
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('MESSAGE_APPLICANT_AFFECTED').count($ids), 'message');
		else
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir);
	}
	
	function delassessor() {
		$user =& JFactory::getUser();
		if(!EmundusHelperAccess::isAdministrator($user->id) && !EmundusHelperAccess::isCoordinator($user->id) && !EmundusHelperAccess::isEvaluator($user->id)) {
		...
		$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('ACTION_DONE'), 'message');
	}
	...
	function unsetAssessor($reqids = null) {
		...
		if (count($ids)>1)
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('MESSAGE_APPLICANTS_UNAFFECTED').count($ids), 'message');
		elseif (count($ids)==1)
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('MESSAGE_APPLICANT_UNAFFECTED').count($ids), 'message');
		else
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir);
	}
	...
	function defaultEmail($reqids = null) {
		...
		if ($error>0)	
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('ACTION_ABORDED'), 'error');
		else 
			$this->setRedirect('index.php?option=com_emundus&view=eval_auto&limitstart='.$limitstart.'&filter_order='.$filter_order.'&filter_order_Dir='.$filter_order_Dir, JText::_('ACTION_DONE'), 'message');
	}
