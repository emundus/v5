...
/**
 *
 * @param FPagination $nav
 * @return unknown
 */
function fabrik_pagination_limitBox(FPagination $nav, $viewAll = false)
{
	...
	if (!in_array($nav->startLimit, $vals))
	{
		$vals[] = $nav->startLimit;
	}
	asort($vals);
	...
	if ($nav->showAllOption == true)
	{
		$limits[] = JHTML::_('select.option', '0', JText::_('COM_FABRIK_ALL'));
	}
	$selected = $viewAll ? 0 : $nav->limit;
	$js = '';
	$attribs = 'class="inputbox" size="1" onchange="' . $js . '"';
	$html = JHTML::_('select.genericlist', $limits, 'limit' . $nav->_id, $attribs, 'value', 'text', $selected);
	return $html;
}
