...
function export_csv($uids, $element_id) {
	...
	if (preg_match('/^(\d+)(.)$/', $memory_limit, $matches)) {
		...
		include_once(JPATH_BASE.'/components/com_emundus/models/list.php');
		require_once (JPATH_COMPONENT.DS.'helpers'.DS.'list.php');
		
		$model = new EmundusModelList;
		$model->getUsers();
		...
		unset($us);
			...
			$query .= ' `#__users`.`id` AS applicant_id
						FROM `#__users` 
						LEFT JOIN `#__emundus_users` ON `#__emundus_users`.`user_id`=`#__users`.`id`';
			$query .= $join_comment;
			$query .= ' WHERE `#__users`.`usertype`="Registered" and `#__users`.`id` IN ('.implode(',', $user_id).') 
	...
	// ********************************************
		...
		// ********************************************
			foreach($user as $key=>$value) {
				...
				if($key != 'final_grade' && $key != 'row_id' && $key != 'user_id'){
			...
			$comment_val= '';
				...
				foreach($elements as $element){
					if(!array_key_exists($element->element_name,$users[0]) || ($element->element_name == 'user_id' && $element->table_name == 'jos_emundus_comments'))	{
						...
						if($element->table_name == 'jos_emundus_comments'){
							$value = '';
							//if($element->table_name == 'jos_emundus_comments' && !empty($value) ) die(print_r($value));
							$c++;
							//die(print_r($comments));
							foreach($comments as $comment){
								if($comment->applicant_id == $user['user_id']) {
								...
								//if(die(print_r($comments));
							}
							
							if($c == $count && !empty($tab_value)){
