...
function return_bytes($val) {
	...
	function export_xls($uids, $element_id) {
			...
			include_once(JPATH_BASE.'/components/com_emundus/models/eval_auto.php');
			
			$model = new EmundusModelEval_auto;
			$model->getUsers();
			die(print_r($model->_applicants));
			$users=$model->_applicants;
			
			$p = new EmundusModelEval_auto;
			$profile = $p->getProfiles();
			
			$col = new EmundusModelEval_auto;
			$column = $col->getTableColumns();
