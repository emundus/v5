...
function export_xls($uids, $element_id) {
		...
		include_once(JPATH_BASE.'/components/com_emundus/models/ranking_auto.php');
		
		$model = new EmundusModelRanking_auto;
		$model->getUsers();
		$users=$model->_applicants;
		
		$p = new EmundusModelRanking_auto;
		$profile = $p->getProfiles();
		
		$col = new EmundusModelRanking_auto;
