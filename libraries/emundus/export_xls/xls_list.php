...
function export_xls($uids, $element_id) {
	...
	// ********************************************
		...
		$appl_cols = $col->getApplicantColumns();
		$filter_cols = $col->getSelectList(); 
		...
		foreach ($header_values as $key=>$value){
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colonne, 1, $value['label']);
			$colonne++;
        }
