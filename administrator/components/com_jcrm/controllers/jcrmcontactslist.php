 ...
 * @subpackage 	Jcrm
 */
class JcrmControllerJcrmcontactslist extends JcrmController{
	...
	public function edit(){
		...
		$view =& $this->getView('jcrmcontacts', 'html');
	...
	public function save(){
		$model = $this->getModel('jcrmcontactslist'); 
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__jcrm_contacts SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_jcrm&controller=jcrmcontactslist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__jcrm_contacts SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'jcrmcontacts' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_jcrm&view=jcrmcontactslist' );
	}
	*/
