 ...
 * @subpackage 	Jcrm
 */
class JcrmModelJcrmcountrieslist extends JModel{
	/**
	 * Jcrmcountrieslist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__jcrm_countries` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,zone,continent'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'name,zone,continent'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__jcrm_countries` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__jcrm_countries` ORDER BY `$fieldName`');
		$options = array();
